...
/*
 * Accepts a stream that has already been synced, and finished processing a
 * batch Finish and cleanup the stream, save primary chain results to unpinned
 * CPU memory. RETURN: number of reads in last batch
 */
int plchain_post_gpu_helper(streamSetup_t stream_setup, int stream_id,
  ...
  kernel_mem_usage[score_kernel_config.micro_batch] =
      (float)(*stream_setup.streams[stream_id].long_mem.total_long_segs_n) /
      stream_setup.long_seg_buffer_size_stream * 100;
  ...
  for (int uid = 0; uid < score_kernel_config.micro_batch; uid++) {
    if (stream_setup.streams[stream_id].host_mems[uid].size == 0)
      continue;
...
#ifdef DEBUG_VERBOSE
    fprintf(stderr,
            "[Debug] %s (%s:%d) MICROBATCH$%d FINISHED: long seg %lu - %u",
    ...
    plchain_backtracking(&stream_setup.streams[stream_id].host_mems[uid],
                         stream_setup.streams[stream_id].reads + n_reads, misc,
                         km);
    // accumulate n_reads
    ...
    fprintf(stderr, ", %.2f%% anchors are in long segs. \n",
            (float)total_n_long_segs /
                stream_setup.streams[stream_id].host_mems[uid].total_n * 100);
#endif // DEBUG_VERBOSE
#endif // DEBUG_PRINT
  }
...
void plchain_cal_score_async(chain_read_t **reads_, int *n_read_, Misc misc,
  ...
  *n_read_ = 0;
  ...
  if (stream_setup.streams[stream_id].busy) {
    stream_setup.streams[stream_id].cudastream->wait();
  ...
  *(stream_setup.streams[stream_id].startevent) = stream_setup.streams[stream_id].cudastream->single_task([=]()-> void{});
...
#ifdef DEBUG_PRINT
  fprintf(stderr,
          "[Info] %s (%s:%d) Launching Batch: n_read %d, total anchors %lu "
          "(mem usage: %.2f%%)\n",
          __func__, __FILE__, __LINE__, n_read, total_n,
          (float)total_n / stream_setup.max_anchors_stream * 100);
#endif // DEBUG_PRINT

  // reset long seg counters
  stream_setup.streams[stream_id].cudastream->memset(
      stream_setup.streams[stream_id].dev_mem.d_long_seg_count, 0,
      sizeof(unsigned int));
  stream_setup.streams[stream_id].cudastream->memset(
...
#ifdef DEBUG_VERBOSE
    fprintf(stderr,
            "[Debug] %s (%s:%d) MICROBATCH#%d batch_n %lu, read_start %d, "
            "read_end %d usage %.2f %%\n",
            __func__, __FILE__, __LINE__, uid, batch_n, read_start, read_end,
            (float)batch_n / stream_setup.max_anchors_stream * 100);
#endif // DEBUG_VERBOS

#ifdef DEBUG_PRINT
    kernel_mem_usage[uid] =
        (float)batch_n / stream_setup.max_anchors_stream * 100;
#endif // DEBUG_PRINT
...
#ifdef DEBUG_CHECK
    planalyze_short_kernel(stream_setup.streams[stream_id], uid,
                           kernel_throughput);
#endif // DEBUG_CHECK
  }
  ...
  if (read_start < n_read) {
    fprintf(stderr,
            "[WARNING] Unable to fit reads %d - %d into a microbatch. Fall "
            "back to cpu chaining\n",
            read_start, n_read - 1);
  }
  ...
  q_ct1
      .memcpy(&num_long_seg,
              stream_setup.streams[stream_id].dev_mem.d_long_seg_count,
              sizeof(unsigned int))
      .wait();
  seg_t *long_segs_og = (seg_t *)malloc(sizeof(seg_t) * num_long_seg);
  q_ct1
      .memcpy(long_segs_og,
              stream_setup.streams[stream_id].dev_mem.d_long_seg_og,
              sizeof(seg_t) * num_long_seg)
      .wait();
...
#ifdef DEBUG_VERBOSE
  auto last_length =
      long_segs_og[map[0]].end_idx - long_segs_og[map[0]].start_idx;
  for (int i = 1; i < num_long_seg; i++) {
    auto this_length =
        long_segs_og[map[i]].end_idx - long_segs_og[map[i]].start_idx;
  ...
  stream_setup.streams[stream_id].dev_mem.d_map =
      sycl::malloc_device<unsigned int>(num_long_seg, q_ct1);
  q_ct1
      .memcpy(stream_setup.streams[stream_id].dev_mem.d_map, map,
 ...
 * [in/out] in_arr_: ptr to array of reads, updated to a batch launched in
 * previous run (NULL if no finishing batch) [in/out] n_read_: ptr to num of
 * reads in array, updated to a batch launched in previous run (NULL if no
 * finishing batch)
 */
void chain_stream_gpu(const mm_idx_t *mi, const mm_mapopt_t *opt,
                      chain_read_t **in_arr_, int *n_read_, int thread_id,
                      void *km) {
...
void finish_stream_gpu(const mm_idx_t *mi, const mm_mapopt_t *opt,
                       chain_read_t **reads_, int *n_read_, int t, void *km) {
  // assume only one seg. and qlen_sum desn't matter
  assert(opt->max_frag_len <= 0);
  Misc misc = build_misc(mi, opt, 0, 1);
